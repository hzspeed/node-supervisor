...
function run (args) {
    ...
    if (!executor) {
        executor = (programExt === "coffee" || programExt === "litcoffee") ? "coffee" :
                   (programExt === "sh") ? "sh" : "node";
    }
...
function crash () {
    ...
    var secondpid = 0;
    if( secondpidFilePath )
        secondpid = parseInt(fs.readFileSync(secondpidFilePath));
    setTimeout(function() {
        if (child) {
            if (instantKillFlag) {
                log("crashing child with SIGKILL");
                console.log("child.pid",child.pid);
                console.log("secondpid",secondpid);
